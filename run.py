...
def load_and_evaluate_model(cfg, dataset, checkpoint_path):

    data_module = hydra.utils.instantiate(cfg['datamodule'], cfg, dataset, _recursive_=False)

    from module.cognn_cross import Net 

    trained_model = Net.load_from_checkpoint(checkpoint_path, cfg=cfg, dataset=dataset, network=cfg['module']['network'], 
    optimizer=cfg['module']['optimizer'], 
    criterion=cfg['module']['criterion'], 
    GAT_params=cfg['module']['GAT_params'])
    trainer = pl.Trainer(accelerator='gpu', devices=1)
    trainer.test(trained_model, datamodule=data_module)
    print(f"Test AUC: {trained_model.test_auc}")
    print(f"Test AUPRC: {trained_model.test_auprc}")
    print(f"Test F1 Score: {trained_model.test_f1}")
...
####################################################################################
    ...
    if cfg['tuning']['param_search']['tune']:
        ...
        analysis = tune.run(
            trainable,
            storage_path="/home/abrar/DTI/DTI-LM-main/hyper_parameters",
        ...
        if cfg['multiprocessing']['multiprocessing']:
            ...
            path =  "/home/abrar/DTI/DTI-LM-main/models/last-v86.ckpt" 
            # load_and_evaluate_model(cfg, dataset, checkpoint_path=path)
