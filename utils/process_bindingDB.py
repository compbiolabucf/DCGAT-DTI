...
def get_data(config):
    ...
    # , nrows= 50000
    data = pd.read_csv(path+'BindingDB_All_202406.tsv',delimiter='\t',on_bad_lines='skip',dtype=str,usecols=[29,1,42,38,8,9,10,11] )
  ...
  #  print(config['label'],"##################", data['Kd (nM)'])
    has_non_nan = data['Kd (nM)'].notna().any()
    print(" ########  Are there any non-NaN values in 'Kd (nM)'?:", has_non_nan, "###############")
    ...
    if config['label'] == 'Kd':
        ...
        #print(" ### INSIDE in get data :", data['label'])
